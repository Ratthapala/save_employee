...
gulp.task('js', () => {
  return gulp.src([ src_assets_folder + 'js/**/*.js' ], { since: gulp.lastRun('js') })
    ...
    .pipe(webpack({
      mode: 'production',
      entry: {
        common: src_assets_folder + '/js/app.js',
        index: src_assets_folder + '/js/index.js'
      },
      output: {
        filename: '[name].js',
        path: __dirname + '/dist'
...
gulp.task('fonts', () => {
  return gulp.src([ src_assets_folder + 'fonts/**/*.+(eot|svg|ttf|woff)' ])
    .pipe(gulp.dest(dist_assets_folder + 'fonts'))
});

gulp.task('video', () => {
  return gulp.src([ src_assets_folder + 'video/**/*.mp4' ])
    .pipe(gulp.dest(dist_assets_folder + 'video'))
});

gulp.task('build', gulp.series('clear', 'html', 'pug', 'sass', 'less', 'stylus', 'js', 'fonts', 'video', 'images', 'vendor'));

gulp.task('dev', gulp.series('html', 'pug', 'sass', 'less', 'stylus', 'js', 'fonts', 'video'));
